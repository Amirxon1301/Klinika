...
class TolovApiView(APIView):
    ...
    def post(self, request):
        serializer = TolovSerializer(validated_data=request.data)
...
class QoshimchaTolovAPIView(APIView):
    def get(self, request):
        ...
        serializer = QoshimchaTolovlarSerializer(q_tolovar, many=True)
        return Response(serializer.data)
    ...
    def post(self, request):
        ...
        summa = tolovlar.aggregate(s=Sum("summa")).get("s")
        data = request.data
        serializer = QoshimchaTolovlarSerializer(validated_data=data)
        if serializer.is_valid():
...
class TolovUpdateDeleteAPIView(APIView):
    def delete(self, request, pk):
        tolovlar = Tolov.objects.get(id=pk)
        ...
        return Response({"xabar":"True"})

    def update(self, request, pk):
        tolov = Tolov.objects.get(id=pk)
        serializer = TolovSerializer(tolov, validated_data=request.data)
        if serializer.is_valid():
            data = serializer.validated_data
            Tolov.objects.filter(id=pk).update(
                tolangan_summa=data.get("tolangan_summa"),
